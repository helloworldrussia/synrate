...
def server_listener():
    if mode == 'short':
        ...
        nelikvidy_obj = ParserNelikvidy(False, 1)
        tender_obj = ParserTender(1)
        roseltorg_obj = RoseltorgParser(1)
        isource_obj = ParserSource(1)
        tectorg_obj = ParserTektorg()
        onlinecontract_obj = ParserOnlineContract(1)
        etpgpb_obj = ParserEtpgpb(1)
        fabrikant_obj = ParserFabrikant(True, 1)
        b2b_center_obj = ParserCenter(True, 1)
        etp_activ_obj = ParserEtpActiv(True, 1)
    else:
